...
import { Search, Filter, ChevronDown, User, Eye, X, Download, Loader2, Pencil, Save, CheckCircle } from 'lucide-react';
...
function getPavColor(pavillon: string | null) { if (!pavillon) return '#94a38b'; const id = pavillon.replace('PAV ', '').trim(); return PAV_COLOR_MAP[id] || '#94a38b'; }

interface FiltersState { search: string; pavillon: string; nationalite: string; niveau: string; paiement: string; sexe: string; }

function Badge({ value, trueLabel = 'نعم', falseLabel = 'لا' }: { value: boolean; trueLabel?: string; falseLabel?: string }) {
  return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${value ? 'bg-emerald-100 text-emerald-700' : 'bg-red-50 text-red-600'}`}>{value ? trueLabel : falseLabel}</span>;
}

function StudentModal({ student, onClose, onEdit }: { student: Student; onClose: () => void; onEdit: () => void }) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="text-white p-5 rounded-t-2xl flex items-center justify-between" style={{ background: `linear-gradient(135deg, ${pavColor}, ${pavColor}dd)` }}>
          <div><h2 className="text-lg font-bold">{student.nom}</h2><p className="text-sm opacity-80">{student.numero_inscription}</p></div>
          <div className="flex gap-2">
            <button onClick={onEdit} className="p-1.5 hover:bg-white/20 rounded-lg" title="تعديل"><Pencil className="w-4 h-4" /></button>
        ...
        <div className="p-5 space-y-3 text-sm" dir="rtl">
          {[
            ['رقم التعريف الوطني', student.nin], ['رقم بكالوريا', student.matricule_bac],
            ['تاريخ الميلاد', student.date_naissance], ['مكان الميلاد', student.lieu_naissance],
            ['الجنس', student.sexe === 'Garçon' ? 'ذكر' : 'أنثى'], ['الجنسية', student.nationalite],
            ['المؤسسة', student.etablissement], ['الميدان', student.domaine],
            ['التخصص', student.filiere], ['المستوى', student.niveau],
            ['الجناح', student.pavillon], ['رقم الغرفة', student.chambre], ['الولاية', student.wilaya_bac],
          ].map(([label, value]) => value ? (
            <div key={label} className="flex justify-between items-center py-1.5 border-b border-slate-100">
              <span className="text-slate-500">{label}</span><span className="font-medium text-slate-700">{value}</span>
            </div>
          ) : null)}
          <div className="flex justify-between items-center py-1.5 border-b border-slate-100">
            <span className="text-slate-500">تسديد مستحقات الإيواء</span><Badge value={student.paiement_hebergement} trueLabel="مسدد" falseLabel="غير مسدد" />
          </div>
          <div className="flex justify-between items-center py-1.5 border-b border-slate-100">
            <span className="text-slate-500">تسليم المفتاح</span><Badge value={student.cle_remise} trueLabel="سُلّم" falseLabel="لم يُسلّم" />
          </div>
          <div className="flex justify-between items-center py-1.5">
            <span className="text-slate-500">رسوم التسجيل</span><Badge value={student.frais_inscription_paye} trueLabel="مدفوعة" falseLabel="غير مدفوعة" />
          </div>
        </div>
        <div className="px-5 pb-4">
          <button onClick={onEdit} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-amber-500 text-white text-sm rounded-lg hover:bg-amber-600 transition-colors">
            <Pencil className="w-4 h-4" />تعديل معلومات الطالب
          </button>
        </div>
      </div>
    </div>
...
function EditModal({ student, onClose, onSaved }: { student: Student; onClose: () => void; onSaved: (s: Student) => void }) {
  ...
  async function handleSave() {
    setSaving(true);
    const update = { ...form, updated_at: new Date().toISOString() };
    const { data, error } = await supabase.from('students').update(update).eq('id', student.id).select().single();
    setSaving(false);
    if (!error && data) { setSaved(true); setTimeout(() => { onSaved(data as Student); onClose(); }, 1000); }
  }

  const field = (label: string, key: keyof Student, type: 'text' | 'date' | 'select' = 'text', options?: string[]) => (
    <div key={key}>
  ...
  const boolField = (label: string, key: keyof Student) => (
    <div key={key} className="flex items-center justify-between py-2 border-b border-slate-100">
      ...
      <button onClick={() => setForm(f => ({ ...f, [key]: !f[key] }))}
        className={`relative w-11 h-6 rounded-full transition-colors ${form[key] ? 'bg-emerald-500' : 'bg-slate-200'}`}>
        <span className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all ${form[key] ? 'right-1' : 'right-6'}`} />
      </button>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="bg-gradient-to-l from-amber-600 to-amber-500 text-white p-5 rounded-t-2xl flex items-center justify-between">
          <div><h2 className="text-lg font-bold">تعديل معلومات الطالب</h2><p className="text-sm opacity-80">{student.nom}</p></div>
          <button onClick={onClose} className="p-1.5 hover:bg-white/20 rounded-lg"><X className="w-5 h-5" /></button>
        </div>
        <div className="p-5 space-y-4" dir="rtl">
          <h4 className="text-xs font-bold text-amber-700 border-b border-amber-100 pb-1">المعلومات الشخصية</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {field('الاسم الكامل', 'nom')}
            {field('رقم التعريف الوطني (NIN)', 'nin')}
            {field('تاريخ الميلاد', 'date_naissance', 'date')}
            {field('مكان الميلاد', 'lieu_naissance')}
            {field('الجنس', 'sexe', 'select', ['Garçon', 'Fille'])}
            {field('الجنسية', 'nationalite')}
          </div>
          <h4 className="text-xs font-bold text-amber-700 border-b border-amber-100 pb-1 mt-2">الإقامة</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {field('الجناح', 'pavillon', 'select', PAVILIONS.map(p => p.label))}
            {field('رقم الغرفة', 'chambre')}
          </div>
          <h4 className="text-xs font-bold text-amber-700 border-b border-amber-100 pb-1 mt-2">المعلومات الأكاديمية</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {field('رقم التسجيل', 'numero_inscription')}
            {field('المؤسسة', 'etablissement')}
            {field('الميدان', 'domaine')}
            {field('الفيلية / التخصص', 'filiere')}
            {field('المستوى', 'niveau', 'select', [...new Set([...levels, form.niveau].filter(Boolean) as string[])])}
          </div>
          <h4 className="text-xs font-bold text-amber-700 border-b border-amber-100 pb-1 mt-2">الحالات</h4>
          {boolField('تسديد مستحقات الإيواء', 'paiement_hebergement')}
          {boolField('تسليم المفتاح', 'cle_remise')}
          {boolField('رسوم التسجيل مدفوعة', 'frais_inscription_paye')}

          <button onClick={handleSave} disabled={saving || saved}
            className="w-full flex items-center justify-center gap-2 py-3 bg-amber-500 text-white rounded-xl hover:bg-amber-600 transition-colors disabled:opacity-60 mt-3 font-medium">
            {saved ? <><CheckCircle className="w-4 h-4" />تم الحفظ بنجاح</> : saving ? <><Loader2 className="w-4 h-4 animate-spin" />جاري الحفظ...</> : <><Save className="w-4 h-4" />حفظ التعديلات</>}
          </button>
        </div>
      </div>
    </div>
...
export default function Students() {
  ...
  const handleExport = useCallback(async () => {
    setExporting(true);
    try { await exportStudentsToExcel(filters); } catch { alert('فشل التصدير. حاول مرة أخرى.'); } finally { setExporting(false); }
  }, [filters]);
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-4">
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="البحث باسم الطالب..." value={filters.search}
              onChange={e => { setFilters(f => ({ ...f, search: e.target.value })); setPage(0); }}
              className="w-full pr-9 pl-3 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-400" dir="rtl" />
          </div>
          <button onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-2 px-4 py-2.5 text-sm rounded-lg border transition-colors ${showFilters ? 'bg-amber-50 border-amber-300 text-amber-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
          ...
          <button onClick={handleExport} disabled={exporting || total === 0}
            ...
            {exporting ? <><Loader2 className="w-4 h-4 animate-spin" />جارٍ التصدير...</> : <><Download className="w-4 h-4" />تصدير Excel</>}
          </button>
        </div>
        {showFilters && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mt-3 pt-3 border-t border-slate-100">
            <select value={filters.pavillon} onChange={e => { setFilters(f => ({ ...f, pavillon: e.target.value })); setPage(0); }} className="text-sm border border-slate-200 rounded-lg px-2 py-2 focus:outline-none focus:ring-2 focus:ring-amber-400">
              <option value="">كل الأجنحة</option>{PAVILIONS.map(p => <option key={p.id} value={p.label}>{p.name}</option>)}
            </select>
            ...
            <select value={filters.niveau} onChange={e => { setFilters(f => ({ ...f, niveau: e.target.value })); setPage(0); }} className="text-sm border border-slate-200 rounded-lg px-2 py-2 focus:outline-none focus:ring-2 focus:ring-amber-400">
              <option value="">كل المستويات</option>{levels.map(n => <option key={n} value={n}>{n}</option>)}
            </select>
                    ...
                    return (
                      <tr key={s.id} className="border-b border-slate-50 hover:bg-amber-50/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {s.pavillon ? <span className="pav-badge text-[10px]" style={{ backgroundColor: pavColor }}>{s.pavillon.replace('PAV ', 'P')}</span> : <span className="text-slate-400">—</span>}
                        ...
                        <td className="px-4 py-3 flex gap-1">
                          <button onClick={() => setSelected(s)} className="p-1.5 hover:bg-amber-50 rounded-lg text-slate-400 hover:text-amber-600 transition-colors"><Eye className="w-4 h-4" /></button>
                          <button onClick={() => setEditing(s)} className="p-1.5 hover:bg-amber-50 rounded-lg text-slate-400 hover:text-amber-600 transition-colors"><Pencil className="w-4 h-4" /></button>
                        </td>
                      </tr>
                    );
                  })}
      ...
      {selected && !editing && <StudentModal student={selected} onClose={() => setSelected(null)} onEdit={() => { setEditing(selected); setSelected(null); }} />}
      {editing && <EditModal student={editing} onClose={() => setEditing(null)} onSaved={handleSaved} />}
    </div>
  );
}
